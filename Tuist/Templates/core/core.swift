...
//  Templates
//
//  Core 모듈 스캐폴드. 직접 부르지 말고 Scripts/new-module.sh 로 실행한다.
...
private let root = "Modules/Core/{{ name }}"

let template = Template(
    description: "Core 모듈(구현, 테스트, 선택: Testing, Demo, Resources)",
    attributes: [
        .required("name"),
    ...
    items: [
        ...
        .file(path: "\(root)/Sources/{{ name }}Placeholder.swift", templatePath: "Placeholder.stencil"),
        .file(path: "\(root)/Tests/{{ name }}Tests.swift", templatePath: "Tests.stencil"),
        .file(path: "\(root)/Testing/Sources/{{ name }}Placeholder+Fixtures.swift", templatePath: "Fixtures.stencil"),
