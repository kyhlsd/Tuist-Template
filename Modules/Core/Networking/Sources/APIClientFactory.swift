...
///
/// 미들웨어는 앞쪽이 바깥쪽이다: Logging → Retry → Auth → transport.
...
public enum APIClientFactory {
    ...
    public static func make(
        ...
        let logging = LoggingMiddleware(subsystem: logSubsystem)

        // refresh 요청은 AuthMiddleware 를 타지 않는 별도 클라이언트로 보낸다. 재귀를 막는다.
        let refreshClient = Client(serverURL: baseURL, transport: transport, middlewares: [logging])
