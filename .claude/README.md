...
- `included:`를 두지 않습니다. 목록에 없는 새 소스 폴더가 전체 검사(CI)에서 조용히
  빠집니다. 전체를 대상으로 두고 `excluded:`만 씁니다.
- `excluded:`의 경로는 루트 기준입니다. `.claude/worktrees/*` 사본은 자기 `Tuist/.build`의
  심볼릭 링크를 따라 `~/.cache`의 외부 패키지까지 검사되므로 `.claude`를 통째로 제외합니다.
- 훅은 `--config` 없이, 편집한 파일이 속한 작업 트리의 루트에서 실행합니다(워크트리 파일이면
  그 워크트리 루트. 위 `.claude` 제외에 걸리지 않습니다). `--config`를 주면 하위 폴더의
