...
### CI (PR에서 측정 예정)
구현은 단계별로 푸시하지 않고 PR을 올릴 때 한 번에 검증하기로 했다(사용자 결정). 아래는 아직 비어 있다.
- 2단계: `build-test`·`release-build` 잡 시간, run 전체 시간
- 3단계: 러너의 기본 CAS 경로(확인 스텝 없이 `$RUNNER_TEMP/cas`로 고정함), `workflow_dispatch` 두 번의
  저장·복원 시간과 CAS 크기, Firebase 타깃 캐시 적중 remark, **결정 게이트**(캐시 적중 run의 `build-test`가 1분 이상 단축되는지)
- 4단계: 커버리지 계측으로 늘어난 테스트 스텝 시간
